...
def test_symmetric_dcp_pairs_ranks_by_token_slice() -> None:
    ...
    keys = topology.get_target_remote_worker_keys(
        remote_tp_size=4, remote_dcp_size=2, remote_pcp_size=1
    )

    # tp ranks 0 and 2 both derive dcp_rank 0. Both are selected because for
    # MLA the head test is vacuous and nothing filters the replica axis --
    # see the TODO(dcp) in _has_kv_cache_overlap_for_local_rank. Ranks 1 and 3
    # cover the other slice and are correctly excluded.
    assert keys == [(0, 0), (0, 2)]
