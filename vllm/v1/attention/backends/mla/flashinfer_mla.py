...
class FlashInferMLAImpl(MLACommonImpl[MLACommonMetadata]):
    ...
    # FlashInfer's trtllm decode kernel returns LSE in base-2 units.
    lse_base_is_e: bool = False
    ...
    def forward_mqa(
        ...
        if not uniform:
            ...
            # Multi-token decode under DCP: split into per-token requests so
            # the kernel's causal offset arithmetic stays correct.
        ...
        # LSE is only needed to reduce partial attention outputs across DCP
        # ranks; request it solely in that case.
